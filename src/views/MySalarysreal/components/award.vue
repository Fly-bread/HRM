            ...
            for(let item of this.$store.state.awardsTb){
                if( beginDate< item.date &&  item.date <= endDate){
                    if(this.userid == item.userid){
                      let temp = JSON.parse(JSON.stringify(item)) //深拷贝，防止修改数据源
                      temp.worknumber = this.formData.worknumber
                      temp.username = this.formData.username
                      this.list.push(temp)

                      //因为total是数字，所以用加法，必须转化
                      this.total += this.typeToNumber(item.awardNum)
                    }
                }
            }
